...
class ClaudeCodeBlock(BlockDefinition):
    ...
    def ui_assets(self, surface: str = "modal") -> list[dict[str, str]]:
        """Return Claude Code owned UI assets for the requested surface."""
    ...
    def _render_modal_tabs(
        ...
        for index, port in enumerate(outputs):
            ...
            panels.append(
                ...
                'placeholder="Decris ce que Claude Code doit produire avec les inputs recus.">'
    ...
    def _render_last_command_panel(
        ...
        *,
        ...
        return (
            ...
            '<h3>Last cmd</h3>'
            '<p>Commande Claude Code preparee par le runtime pour la derniere execution.</p>'
            '</div>'
            ...
            '</div>'
            f'<p class="claude-last-command-empty{empty_class}">Aucune commande Claude Code enregistree pour ce bloc.</p>'
